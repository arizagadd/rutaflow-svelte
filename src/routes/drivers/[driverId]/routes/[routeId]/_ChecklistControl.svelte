<script>
    ...
    const sendEvidence = async () => {
        ...
        if (isLast) {
            ...
            let coords = null;
            try {
                coords = await getCurrentCoords(); // ← await aquí
            } catch (e) {
            ...
            checklistStore.update((store) => {
                ...
                const mandatoryItems = store.items.filter(
                    (item, index) => store.mandatory[index] === "1"
                );
                // Check if all mandatory items are checked
                const allMandatoryChecked = mandatoryItems.every(
                    (mandatoryItem) =>
                ...
                <ion-item>
                    <ion-label style="display:flex;">
                        <input
                            ...
                            on:change={() =>
                                markCheckbox(check.id_checklist_event)}
                            required
                        />
