...
function s(e2, t2, n2) {
  ...
  if (e2.constructor === Object) {
    ...
    var p2 = {};
    for (var f2 in n2[l2] = p2, e2)
      p2[f2] = s(e2[f2], t2, n2);
    return Object.setPrototypeOf(p2, t2[h2].prototype), p2;
  }
}
function u(e2) {
  o[e2.name] = e2;
...
function p(e2, t2) {
  const n2 = indexedDB.open(e2);
  n2.onupgradeneeded = () => n2.result.createObjectStore(t2);
  const r2 = d(n2);
...
function P() {
  if ("object" != typeof indexedDB || !k("indexedDB"))
    return w("IndexedDB"), T();
  const { removeListener: e2, callListeners: t2, addListener: n2 } = N(), r2 = p("svelte-persist", "persist");
  return { addListener: n2, removeListener: e2, getValue: (e3) => (function(e4, t3 = y()) {
    return t3("readonly", (t4) => d(t4.get(e4)));
  }(e3, r2).then((n3) => t2(e3, S(n3))), null), setValue(e3, t3) {
    !function(e4, t4, n3 = y()) {
      n3("readwrite", (n4) => (n4.put(t4, e4), d(n4.transaction)));
    }(e3, A(t3), r2);
  }, deleteValue(e3) {
    !function(e4, t3 = y()) {
      t3("readwrite", (t4) => (t4.delete(e4), d(t4.transaction)));
    }(e3, r2);
  } };
}
var M;
function B(e2 = M.LOCAL, t2 = false) {
  if ("object" != typeof chrome || "object" != typeof chrome.storage)
    return w("ChromeStorage"), T();
  let n2 = "local";
  switch (e2) {
    case M.LOCAL:
      n2 = "local";
  ...
  function r2(e3, t3) {
    if (t3 === n2)
      for (const [t4, { newValue: n3 }] of Object.entries(e3))
        i2(t4, n3);
  }
  const { removeListener: o2, callListeners: i2, addListener: c2 } = N(() => {
    t2 && chrome.storage.onChanged.addListener(r2);
  }, () => {
    t2 && chrome.storage.onChanged.removeListener(r2);
  });
  return { addListener: c2, removeListener: o2, getValue: (e3) => (chrome.storage[n2].get([e3], (t3) => i2(e3, t3.key)), null), setValue(e3, t3) {
    chrome.storage[n2].set({ [e3]: t3 });
  }, deleteValue(e3) {
    chrome.storage[n2].remove(e3);
  } };
}
function U() {
...
function q(e2, t2) {
  return j(e2, G("cookie"), t2);
}
function F(e2, t2) {
  return j(e2, G("session"), t2);
}
function J(e2, t2) {
...
ae.cipher.aes = function(e2) {
  ...
  var e2, t2, n2, r2, o2, i2, c2, a2, s2 = this._tables[0], u2 = this._tables[1], l2 = s2[4], f2 = u2[4], h2 = [], d2 = [];
  for (e2 = 0; e2 < 256; e2++)
    d2[(h2[e2] = e2 << 1 ^ 283 * (e2 >> 7)) ^ e2] = e2;
  for (t2 = n2 = 0; !l2[t2]; t2 ^= r2 || 1, n2 = d2[n2] || 1)
  ...
  var n2, r2, o2, i2, c2 = this._key[t2], a2 = e2[0] ^ c2[0], s2 = e2[t2 ? 3 : 1] ^ c2[1], u2 = e2[2] ^ c2[2], l2 = e2[t2 ? 1 : 3] ^ c2[3], f2 = c2.length / 4 - 2, h2 = 4, d2 = [0, 0, 0, 0], p2 = this._tables[t2], g2 = p2[0], y2 = p2[1], m2 = p2[2], b2 = p2[3], v2 = p2[4];
  for (i2 = 0; i2 < f2; i2++)
    n2 = g2[a2 >>> 24] ^ y2[s2 >> 16 & 255] ^ m2[u2 >> 8 & 255] ^ b2[255 & l2] ^ c2[h2], r2 = g2[s2 >>> 24] ^ y2[u2 >> 16 & 255] ^ m2[l2 >> 8 & 255] ^ b2[255 & a2] ^ c2[h2 + 1], o2 = g2[u2 >>> 24] ^ y2[l2 >> 16 & 255] ^ m2[a2 >> 8 & 255] ^ b2[255 & s2] ^ c2[h2 + 2], l2 = g2[l2 >>> 24] ^ y2[a2 >> 16 & 255] ^ m2[s2 >> 8 & 255] ^ b2[255 & u2] ^ c2[h2 + 3], h2 += 4, a2 = n2, s2 = r2, u2 = o2;
  for (i2 = 0; i2 < 4; i2++)
    d2[t2 ? 3 & -i2 : i2] = v2[a2 >>> 24] << 24 ^ v2[s2 >> 16 & 255] << 16 ^ v2[u2 >> 8 & 255] << 8 ^ v2[255 & l2] ^ c2[h2++], n2 = a2, a2 = s2, s2 = u2, u2 = l2, l2 = n2;
  return d2;
  ...
  var c2, a2, s2, u2, l2, f2, h2, d2, p2, g2, y2, m2, b2 = ae.bitArray;
  for (p2 = n2.length, g2 = b2.bitLength(n2), y2 = b2.bitLength(r2), m2 = b2.bitLength(o2), c2 = t2.encrypt([0, 0, 0, 0]), 96 === m2 ? (a2 = o2.slice(0), a2 = b2.concat(a2, [1])) : (a2 = ae.mode.gcm._ghash(c2, [0, 0, 0, 0], o2), a2 = ae.mode.gcm._ghash(c2, a2, [0, 0, Math.floor(m2 / 4294967296), 4294967295 & m2])), s2 = ae.mode.gcm._ghash(c2, [0, 0, 0, 0], r2), f2 = a2.slice(0), h2 = s2.slice(0), e2 || (h2 = ae.mode.gcm._ghash(c2, s2, n2)), l2 = 0; l2 < p2; l2 += 4)
    f2[3]++, u2 = t2.encrypt(f2), n2[l2] ^= u2[0], n2[l2 + 1] ^= u2[1], n2[l2 + 2] ^= u2[2], n2[l2 + 3] ^= u2[3];
  return n2 = b2.clamp(n2, g2), e2 && (h2 = ae.mode.gcm._ghash(c2, s2, n2)), d2 = [Math.floor(y2 / 4294967296), 4294967295 & y2, Math.floor(g2 / 4294967296), 4294967295 & g2], h2 = ae.mode.gcm._ghash(c2, h2, d2), u2 = t2.encrypt(a2), h2[0] ^= u2[0], h2[1] ^= u2[1], h2[2] ^= u2[2], h2[3] ^= u2[3], { tag: b2.bitSlice(h2, 0, i2), data: n2 };
} }, function(e2) {
...
var ue = class {
  constructor(e2) {
    this.cipher = new ae.cipher.aes(ce(e2));
  }
  encrypt(e2, t2) {
    t2 || (t2 = "sps");
    const n2 = ae.codec.utf8String.toBits(t2);
    return ne(ae.mode.gcm.encrypt(this.cipher, ae.codec.utf8String.toBits(e2), n2, [], 256)) + ":" + ne(n2);
  }
  decrypt(e2) {
    return ae.codec.utf8String.fromBits(ae.mode.gcm.decrypt(this.cipher, ce(e2.split(":")[0]), ce(e2.split(":")[1])));
  }
  hash(e2) {
    return this.encrypt(e2, "sps");
  }
};
function le(e2, t2) {
  return fe(e2, new ue(t2));
}
function fe(e2, t2) {
  return { getValue(n2) {
    const r2 = t2.hash(n2), o2 = e2.getValue(r2);
    return null === o2 ? null : S(t2.decrypt(o2));
  }, setValue(n2, r2) {
    const o2 = t2.hash(n2), i2 = t2.encrypt(A(r2));
    e2.setValue(o2, i2);
  }, deleteValue(n2) {
    const r2 = t2.hash(n2);
    e2.deleteValue(r2);
...
export {
  ue as GCMEncryption,
  O as addSerializableClass,
  Q as cookieWritable,
  B as createChromeStorage,
  R as createCookieStorage,
  le as createEncryptedStorage,
  fe as createEncryptionStorage,
  P as createIndexedDBStorage,
  I as createLocalStorage,
  U as createNoopStorage,
  V as createSessionStorage,
  b as disableWarnings,
  X as localWritable,
  se as noEncryptionBehavior,
  j as persist,
  J as persistBrowserLocal,
  F as persistBrowserSession,
  q as persistCookie,
  K as sessionWritable,
  E as setSerialization,
  H as writable
};
//# sourceMappingURL=@macfja_svelte-persistent-store.js.map
