import { c as create_ssr_component, d as subscribe, v as validate_component, e as escape$1, a as add_attribute, f as each } from "../../../../chunks/ssr.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-qavk4j_START -->${$$result.title = `<title>Rutas - Rutaflow</title>`, ""}<!-- HEAD_svelte-qavk4j_END -->`, ""} ${dataSession ? `${validate_component(IonPage, "IonPage").$$render($$result, {}, {}, {
    default: () => {
      return `<ion-header translucent><ion-toolbar><ion-buttons slot="secondary"><ion-button>${escape$1(dataSession.name)}</ion-button></ion-buttons> <ion-title data-svelte-h="svelte-1oi1i0m">RUTAS DEL DÍA</ion-title> <ion-buttons slot="primary"><ion-button title="Salir" alt="Salir" data-svelte-h="svelte-1itonhq"><ion-icon${add_attribute("icon", logOut, 0)}></ion-icon></ion-button></ion-buttons></ion-toolbar></ion-header> <ion-content><ion-refresher slot="fixed"${add_attribute("this", refresher, 0)} data-svelte-h="svelte-1wkherr"><ion-refresher-content pulling-icon="arrow-dropdown" pulling-text="Jale para actualizar" refreshing-spinner="circles" refreshing-text="Actualizando..."></ion-refresher-content></ion-refresher> <ion-list>${driverId == "me" && flag ? `${each(routes, (route) => {
        return `${compareDates(route.date_start) ? `${escape$1(changePendingState("true"))} <ion-item button><ion-avatar slot="start"><div class="route-color svelte-1xh3ip" style="${"background-color:" + escape$1(getDeliveryColor(route.status), true) + ";color: " + escape$1(getContrast(getDeliveryColor(route.status)), true)}"${add_attribute("title", setTitleStatus(route.status), 0)}><div class="route-symbol svelte-1xh3ip" style="">${escape$1(capitalizeFirstLetter(route.name.charAt(0)))}</div> </div></ion-avatar> <ion-label><h2>${escape$1(route.name ? route.name.toUpperCase() : "")}</h2> <h3 class="text-muted svelte-1xh3ip">Inicio: ${escape$1(route.date_start)}</h3></ion-label> <div slot="end"></div> </ion-item>` : `${escape$1(changePendingState("false"))}`}`;
      })}` : ` ${each(routes.filter((route) => route.id_driver === driverId), (route) => {
        return `${compareDates(route.date_start) ? `${escape$1(changePendingState("true"))} ${route.status != "completed" && route.status != "cancelled" ? `<ion-item button><ion-avatar slot="start"><div class="route-color svelte-1xh3ip" style="${"background-color:" + escape$1(getDeliveryColor(route.status), true) + ";color: " + escape$1(getContrast(getDeliveryColor(route.status)), true)}"${add_attribute("title", setTitleStatus(route.status), 0)}><div class="route-symbol svelte-1xh3ip" style="">${escape$1(capitalizeFirstLetter(route.name.charAt(0)))}</div> </div></ion-avatar> <ion-label><h2>${escape$1(route.name)}</h2> <h3 class="text-muted svelte-1xh3ip">Inicio: ${escape$1(route.date_start)}</h3></ion-label> <div slot="end"></div> </ion-item>` : `${escape$1(changePendingState("false"))}`}` : `${escape$1(changePendingState("false"))}`}`;
